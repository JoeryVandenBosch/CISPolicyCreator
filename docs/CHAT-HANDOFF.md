...
**Updated:** 2026-08-18
**Current delivery:** PR #10, `Add fail-closed policy JSON bundle importer`
...
Do not describe every supported benchmark as a complete CIS baseline. Five catalogs
are complete **for policy creation** because every recommendation has a final mapping
state and every actual deterministic Intune setting can produce JSON after required
administrator input. Windows 11 v5.0.0 remains a clearly identified partial catalog.
Human/process recommendations correctly produce no fake policy JSON.
...
These are the exact public catalog counts and expected JSON output counts as of PR #10:
...
The four complete catalogs with organizational questions (Windows 10, Edge, macOS, and
...
5. resolves every typed Graph object by exactly one SHA-256-bound repository contract;
6. rejects unexpected properties, types, endpoints, operations, or assignment data;
7. reads all same-name objects and proves an exact expected-property match;
8. aborts before any write on a duplicate, ambiguous, unreadable, or different collision.
...
The iOS/iPadOS bundle contains 59 assignable policy objects plus two Intune-wide
...
## Validation completed for PR #10

- All five complete generated ZIPs passed offline importer preparation:
  ...
  - iOS/iPadOS: 7 Settings Catalog policies, 52 typed Graph policies, and 2 guarded
    tenant-wide values.
- A live read-only Edge dry run validated all 138 Settings Catalog policies in the
  pinned test tenant.
- A live read-only iOS/iPadOS dry run validated all 61 objects/values in the same test
  tenant.
...
- The final `Test-CISRepository.ps1 -PythonPath .\.venv\Scripts\python.exe
  -RequireGraph` run passed with PowerShell 7.6.4, Python 3.14.0, the pinned Graph
  module, 12 Python tests, synthetic PDF extraction, deterministic build/export, bundle
  validation, and offline importer preparation.
- Both GitHub Actions validation jobs for PR #10 passed before its documentation update;
  rerun checks must pass again before merge.
- Every importer test above was offline or read-only. PR #10 intentionally created no
  Intune policies, changed no singleton values, and created no assignments. Earlier
  repository work separately proved unassigned policy creation through the pack
  importer, but that is not presented as a live write test of the new ZIP importer.
...
- completed policy-creation catalogs for the five selected non-Windows-11 PDFs;
...
- offline, live read-only, local regression, and CI validation described above.

No implementation task remains after PR #10 unless the user explicitly requests one.
Possible future work is optional and must not be silently started: run an actual ZIP
import in a disposable test tenant, add newly released exact CIS Intune benchmark
