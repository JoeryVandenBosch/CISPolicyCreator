...
11. Create policies unassigned only after all selected mappings and payloads pass preflight.
...
- A generic Graph object must match a SHA-256-bound repository contract for its exact endpoints, type, and writable primitive properties. A unique same-name object is skipped only after every expected payload property compares exactly; duplicates, missing properties, drift, or an unknown property abort before writes. Existing objects are never patched.
